$(function () {
	...
	function getListDayMeal(day, part){
		...
		getdataprint = '&day=' + day + '&day_part=' + part;
		$.ajax({
            url: baseUrl,
            type: 'POST',
            data: {
                ...
                try {
					...
					for (i in obj) {
						if(obj[i].items){
							...
							listMeals[obj[i].meal_id].haveContent = false;
							//count++;
						}
						if(obj[i].place_name){
						...
						if(obj[i].number){
							...
							if(itemPlaceMeals.count>0)
								listMeals[itemPlaceMeals.meal_id].haveContent = true;
							listPlaceMeals.push(itemPlaceMeals);
						}
					}
					
					for (i in listMeals) {
						if(listMeals[i].haveContent)
							count++;
					}
					...
					if(!autoload){
						tbl +='<tr class="food_f_footertbl" style="border: 1px solid #727272;">'
							...
							+ '<img style="margin: 0;padding: 0;display: block;" src="' + parentUrl + 'img/savehafteebadi.gif"/></div>'
					...
					if($('#li-food-tabs-4').attr('permit')==1)
						if(taeed == 1){
	...
	$('body').on('click', '#print_report', function () { 
		var win = window.open(baseUrl + "?r=" + Math.random() + "&act=print1" + getdataprint, '_blank');
		if(win){
                ...
                try {
					...
					for (i in obj) {
						if(obj[i].id){
							var user_name = {};
							//userList.[obj[i].id] = {};
							user_name.id = obj[i].id;
							user_name.name = obj[i].name;
					...
					for(i in userList){
						sel += '<option value="'+ userList[i].id +'">' +userList[i].name + ' </option>';
					}
	...
	var autoload = $('#food_report_table').attr('auto');
	if(autoload){
		//alert('sfdf');
		baseUrl = "";
		parentUrl = "../../../"
		var da = $('#food_report_table').attr('day');
		var pa = $('#food_report_table').attr('part');
		getListDayMeal(da,pa);
	}
});